...
for (let i = 0; i < 22 && !done; i++) {
  ...
  const info = await page.evaluate(() => {
    ...
    const slideIcons = Array.from(document.querySelectorAll('[data-testid="preview-panel"] foreignObject i[class*="fa-"]'))
      .map(el => el.getAttribute('class'));
    return { isIconSlide: /becomes an icon/i.test(txt), slideIcons };
  });
  if (info.isIconSlide) {
    console.log('ON ICON SLIDE. Icons rendered in slide:', info.slideIcons.length ? info.slideIcons.join(', ') : 'NONE');

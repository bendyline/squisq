...
function getTemplateDefaults(
  ...
  switch (templateName) {
    ...
    case 'fullBleedQuote':
    case 'pullQuote':
      // These two take `text`, not `quote` — `deriveTemplateInputs` owns the
      // per-template input keys. Emitting `quote` here left the templates'
      // impact-text layer with no text at all, so the slide rendered blank in
      // the player and exported as an empty slide.
      return (
        deriveTemplateInputs(templateName, headingText, block.contents) ?? {

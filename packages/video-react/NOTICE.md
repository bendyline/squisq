...
The core runtime is distributed separately from this package's MIT-licensed
Squisq code. A verbatim copy of GPLv2 is included as
`COPYING.GPL-2.0.txt`. Hosts that publish `ffmpeg-core.js` or
`ffmpeg-core.wasm` must preserve the applicable notices, provide the GPL text,
and provide equivalent access to the complete corresponding source for the
exact binaries.
...
The `@ffmpeg/ffmpeg` JavaScript wrapper and `@ffmpeg/util` package are separate
MIT-licensed components. Copyright and complete license texts for other

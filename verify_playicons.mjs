...
for (let i = 0; i < 22; i++) {
  ...
  const info = await page.evaluate(() => {
    const txt = (document.querySelector('[data-testid="preview-panel"]')?.innerText||'').replace(/\s+/g,' ').trim();
    const icons = Array.from(document.querySelectorAll('[data-testid="preview-panel"] foreignObject i[class*="fa-"]'));
    return { isIconSlide: /becomes an icon/i.test(txt),
      icons: icons.map(el=>({cls:el.getAttribute('class'), disp:getComputedStyle(el).display,
        x: Math.round(el.getBoundingClientRect().left), y: Math.round(el.getBoundingClientRect().top)})) };
  });
  if (info.isIconSlide) {
    console.log('icons on slide:');
    info.icons.forEach(ic=>console.log('  ', ic.cls, 'display='+ic.disp, 'at('+ic.x+','+ic.y+')'));

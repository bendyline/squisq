...
const h2c = fs.readFileSync(
  '/Users/mike/gh/squisq/node_modules/html2canvas/dist/html2canvas.min.js',
  'utf8',
);
...
for (let i = 0; i < 22; i++) {
  ...
  const isIcon = await page.evaluate(() =>
    /becomes an icon/i.test(
      document.querySelector('[data-testid="preview-panel"]')?.innerText || '',
    ),
  );
  if (isIcon) break;
  if (
...
const result = await page.evaluate(async () => {
  ...
  const el =
    document.querySelector('[data-testid="preview-panel"] .doc-player') ||
